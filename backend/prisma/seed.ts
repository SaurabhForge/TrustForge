...
async function main() {
  console.log('🌱 Seeding TrustForge database...');
  ...
  try {
    ...
    await prisma.quorumRequest.deleteMany();
    ...
    await Promise.all(permissions.map((p) => prisma.rolePermission.create({ data: { roleId: adminRole.id, permissionId: p.id } })));
    const secAdminPerms = permissions.filter((p) => !['ROLE_ASSIGN', 'ROLE_REVOKE'].includes(p.code));
    await Promise.all(secAdminPerms.map((p) => prisma.rolePermission.create({ data: { roleId: secAdminRole.id, permissionId: p.id } })));
    const auditorPerms = permissions.filter((p) => ['AUDIT_VIEW', 'IDENTITY_VERIFY', 'ASSET_VERIFY'].includes(p.code));
    await Promise.all(auditorPerms.map((p) => prisma.rolePermission.create({ data: { roleId: auditorRole.id, permissionId: p.id } })));
    const verifierPerms = permissions.filter((p) => ['IDENTITY_VERIFY', 'ASSET_VERIFY', 'AUDIT_VIEW'].includes(p.code));
    await Promise.all(verifierPerms.map((p) => prisma.rolePermission.create({ data: { roleId: verifierRole.id, permissionId: p.id } })));
    const userPerms = permissions.filter((p) => ['AUDIT_VIEW'].includes(p.code));
    await Promise.all(userPerms.map((p) => prisma.rolePermission.create({ data: { roleId: userRole.id, permissionId: p.id } })));
